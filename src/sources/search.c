    ...
    bool cutNode)
{
    ...
    // Internal iterative reductions. If the node is not likely to fail low, but a ttMove is not
    // present, reduce depth.
    if (!rootNode && (pvNode || cutNode) && ttMove == NO_MOVE && depth >= 4) --depth;
